...
public class WeightScore {
    ...
    /*
     * The method checks whether the input percentages will add up to 100.
     * will return true if the total percentage is over 100
     */
    public boolean check_perc(double perc) {
        boolean check = true;
        if (perc>100) {
            check = true;
        }
    ...
    /* The method calculates and returns the final weighted grade if the percentage total is not larger
     * than 100. Otherwise it will return -1 for total grade and warn users to check the input.
    ...
    public double weightGrade (double point, double earned, double perc) {
        ...
        earns = w.getEarned();
        percents = w.getPerc();
        if (check_perc(perc) == true){
            totals = -1;
            System.out.println("percentage should add up to 100");
        }
        else {
